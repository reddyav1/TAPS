...
def conv7x7(in_planes, out_planes, stride=1, groups=1, dilation=1):
    ...
    return freeze_conv(in_planes, out_planes, kernel_size=7, stride=stride,
...
class freeze_norm(nn.BatchNorm2d):
    def __init__(self, conv, thresh, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.conv = conv
        self.frozen_mean = self.running_mean.clone()
        self.frozen_var = self.running_var.clone()
        self.thresh = thresh
    
    def forward(self, x):
        if self.conv.indicator <= self.thresh:
            x = (x-self.frozen_mean)/(self.frozen_var+self.eps)**(1/2)
        else:
